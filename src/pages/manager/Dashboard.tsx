...
const RANGES: { label: string; filter: FilterType; active: string; inactive: string }[] = [
    ...
    { label: 'Kecha',  filter: 'yesterday', active: 'bg-emerald-50 text-white border-emerald-200',       inactive: 'bg-emerald-50 text-emerald-600 border-emerald-200 hover:bg-emerald-50' },
    { label: '7 kun',  filter: 'last7',     active: 'bg-emerald-50 text-white border-emerald-200',   inactive: 'bg-emerald-50 text-emerald-600 border-emerald-200 hover:bg-emerald-50' },
        ...
        <div className="space-y-5">
            ...
            {statusLoading ? (
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    {[
                        { label: t('Jami xodimlar', language), value: status?.totalUsers ?? 0, icon: Users, color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-200', light: 'bg-emerald-50' },
                        { label: t('Filiallar', language), value: status?.totalBranches ?? 0, icon: Building2, color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-200', light: 'bg-emerald-50' },
                        ...
                        {roomStats.map((r, i) => (
                            <div key={r.name} className="flex items-center gap-3 p-3.5 rounded-2xl border border-border/60 bg-muted/20 hover:bg-muted/40 transition-colors">
                                <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 text-sm font-bold
                                    ${i === 0 ? 'bg-amber-100 text-amber-700' : i === 1 ? 'bg-slate-100 text-slate-600' : i === 2 ? 'bg-emerald-50 text-emerald-600' : 'bg-emerald-50 text-emerald-600'}`}>
                                    {i + 1}
                                </div>
